...
public class BarLineChartViewBase: ChartViewBase, UIGestureRecognizerDelegate
{
    ...
    private var linehorizontalLabel = UILabel()
    private var LabelA = UILabel()
    private var LabelB = UILabel()
    private var LabelC = UILabel()
    private var LabelD = UILabel()
    private var Labelval = UILabel()
    ...
    private var _LongGestureRecognizer: UILongPressGestureRecognizer!
    var touch:CGPoint!
    ...
    internal override func initialize()
    {
        ...
        _LongGestureRecognizer = UILongPressGestureRecognizer(target: self, action: Selector("longGestureRecognized:"));
        _LongGestureRecognizer.minimumPressDuration = 0.1;
        
        self.lineVertical = UIView(frame: CGRectMake(0, 0, 1, self.bounds.size.height - 23))
        self.lineVertical.backgroundColor = UIColor.blackColor()
        self.lineVertical.hidden = true
        self.addSubview(self.lineVertical)
        
        self.linehorizontal = UIView(frame: CGRectMake(39, 0, self.bounds.size.width,1))
        self.linehorizontal.backgroundColor = UIColor.blackColor()
        self.linehorizontal.hidden = true
        self.addSubview(self.linehorizontal)
        
        self.linehorizontalLabel = UILabel(frame: CGRectMake(5, 0, 32,15))
        self.linehorizontalLabel.text = "12.22"
        self.linehorizontalLabel.textAlignment = .Center
        self.linehorizontalLabel.font = UIFont(name: "HelveticaNeue", size:11)
        self.linehorizontalLabel.textColor = UIColor.whiteColor()
        self.linehorizontalLabel.backgroundColor = UIColor.lightGrayColor()
        self.linehorizontalLabel.hidden = true
        self.linehorizontalLabel.layer.cornerRadius = 15.0
        self.addSubview(self.linehorizontalLabel)
        
        self.LabelA = UILabel(frame: CGRectMake(40, 0, 50,15))
        self.LabelA.text = "开:13.88"
        self.LabelA.textAlignment = .Left
        self.LabelA.font = UIFont(name: "HelveticaNeue", size:11)
        self.LabelA.textColor = UIColor.blackColor()
        self.LabelA.hidden = true
        self.addSubview(self.LabelA)
        
        self.LabelB = UILabel(frame: CGRectMake(85, 0, 50,15))
        self.LabelB.text = "收:13.88"
        self.LabelB.textAlignment = .Left
        self.LabelB.font = UIFont(name: "HelveticaNeue", size:11)
        self.LabelB.textColor = UIColor.blackColor()
        self.LabelB.hidden = true
        self.addSubview(self.LabelB)
        
        self.LabelC = UILabel(frame: CGRectMake(130, 0, 50,15))
        self.LabelC.text = "低:13.88"
        self.LabelC.textAlignment = .Left
        self.LabelC.font = UIFont(name: "HelveticaNeue", size:11)
        self.LabelC.textColor = UIColor.blackColor()
        self.LabelC.hidden = true
        self.addSubview(self.LabelC)
        
        self.LabelD = UILabel(frame: CGRectMake(175, 0, 50,15))
        self.LabelD.text = "高:13.88"
        self.LabelD.textAlignment = .Left
        self.LabelD.font = UIFont(name: "HelveticaNeue", size:11)
        self.LabelD.textColor = UIColor.blackColor()
        self.LabelD.hidden = true
        self.addSubview(self.LabelD)
    
//        let defaults = NSUserDefaults.standardUserDefaults()
//        var timeY: AnyObject? = defaults.objectForKey("gridLineBuffer")
//        var timedy = timeY!.doubleValue
//        var timeYY = CGFloat(timedy)
        
        self.Labelval = UILabel(frame: CGRectMake(43, self.bounds.size.height-25, 150,15))
        self.Labelval.text = "成交量:138万"
        self.Labelval.textAlignment = .Left
        self.Labelval.font = UIFont(name: "HelveticaNeue", size:9)
        self.Labelval.textColor = UIColor.blackColor()
        self.Labelval.hidden = true
    ...
    @objc private func tapGestureRecognized(recognizer: UITapGestureRecognizer)
    {
        ...
        delegate!.doubleTapGestureRecognizedDelegate!(self);
        ...
        if (recognizer.state == UIGestureRecognizerState.Began)
        {
            if (!_dataNotSet && (_pinchZoomEnabled || _scaleXEnabled || _scaleYEnabled))
            {
                ...
                else
                {
                    ...
                    if (x > y)
                    {
                        _gestureScaleAxis = .X;//X轴放大
                    }
                    else
                    {
                        _gestureScaleAxis = .Y;//Y轴放大
                    }
                }
            }
        }
    ...
    //MARK:长按手势显示十字线
    @objc private func longGestureRecognized(recognizer: UILongPressGestureRecognizer){
        ...
        let lowobject: AnyObject? = defualts.objectForKey("low")
        let highobject: AnyObject? = defualts.objectForKey("high")
        let openobject: AnyObject? = defualts.objectForKey("open")
        let closeobject: AnyObject? = defualts.objectForKey("close")
        let valobject: AnyObject? = defualts.objectForKey("val")
        let middleobject: AnyObject? = defualts.objectForKey("middle")
        let horzYobject: AnyObject? = defualts.objectForKey("horzY")
        
        let middledouble = middleobject?.doubleValue
        let lowdouble = lowobject?.doubleValue
        let highdouble = highobject?.doubleValue
        let opendouble = openobject?.doubleValue
        let closedouble = closeobject?.doubleValue
        let horzYdouble = horzYobject?.doubleValue
        let valdouble = valobject?.doubleValue
        
        if (middledouble == nil){
            middledouble == 1;
        }
        println("\(middledouble)")
        
//        let middle = round(middledouble!*100)/100
//        let low = round(lowdouble!*100)/100
//        let high = round(highdouble!*100)/100
//        let open = round(opendouble!*100)/100
//        let close = round(closedouble!*100)/100
//        let horzY = round(horzYdouble!*100)/100
//        let val = round(valdouble!*100000)/100000
        
        var location = recognizer.locationInView(self);
        if(recognizer.state == UIGestureRecognizerState.Began){
            
//            self.lineVertical.frame = CGRectMake(location.x, 0, 1, self.bounds.size.height - 23)
//            self.lineVertical.hidden = false
//            
//            self.linehorizontal.frame = CGRectMake(39, CGFloat(horzY), self.bounds.size.width, 1)
//            self.linehorizontal.hidden = false
//            print("Y1:\(location.y)")
//            self.linehorizontalLabel.frame = CGRectMake(5, CGFloat(horzY)-7.5, CGRectGetWidth(self.linehorizontalLabel.frame), CGRectGetHeight(self.linehorizontalLabel.frame))
//            self.linehorizontalLabel.hidden = false
////            self.linehorizontalLabel.text = "\(middle)"
//            
//            self.LabelA.hidden = false
//            self.LabelA.text = "开:\(open)"
//            self.LabelB.hidden = false
//            self.LabelB.text = "收:\(close)"
//            self.LabelC.hidden = false
//            self.LabelC.text = "低:\(low)"
//            self.LabelD.hidden = false
//            self.LabelD.text = "高:\(high)"
//            self.Labelval.hidden = false
//            self.Labelval.text = "成交量:\(val)万"
            
        }else if(recognizer.state == UIGestureRecognizerState.Changed){
            
            // 超出范围隐藏线
            if(location.y > self.bounds.height - 24 || location.y < 7 || location.x < 40){
                self.linehorizontal.hidden = true
                self.linehorizontalLabel.hidden = true
            }else{
                self.linehorizontal.hidden = false
