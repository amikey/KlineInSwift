...
//                var label = _xAxis.values[maxx - i];//时间从右到左
                ...
                ChartUtils.drawText(context: context, text: label, point: CGPoint(x: position.x, y: timeYY), align: .Left, attributes: [NSFontAttributeName: labelFont, NSForegroundColorAttributeName: labelTextColor]);
            }
        }
    }
...
//        for (var i = _minX; i <= _maxX; i += _xAxis.axisLabelModulus)
        for (var i = _maxX; i >= _minX; i -= _xAxis.axisLabelModulus)
        {
//            position.x = CGFloat(i);
            position.x = CGFloat(i-1);
            position.y = 0.0;
            position = CGPointApplyAffineTransform(position, valueToPixelMatrix);
