...
public class ChartYAxisRenderer: ChartAxisRendererBase
{
    ...
    internal func drawYLabels(#context: CGContext, fixedPosition: CGFloat, offset: CGFloat, textAlign: NSTextAlignment)
    {
        ...
        {
            var text = _yAxis.getFormattedLabel(i+1);
            ...
            pt.x = 0;
            pt.y = CGFloat(_yAxis.entries[i]) + 7;//加上一个值，改变起点
            pt = CGPointApplyAffineTransform(pt, valueToPixelMatrix);
            
            pt.x = fixedPosition;
            pt.y += offset;
            
            ChartUtils.drawText(context: context, text: text, point: pt, align: .Right, attributes: [NSFontAttributeName: labelFont, NSForegroundColorAttributeName: labelTextColor]);
            ...
            if i == 0{
                ...
                pt.x = fixedPosition;
                pt.y += offset;
                 ChartUtils.drawText(context: context, text: "万 手", point: pt, align: .Right, attributes: [NSFontAttributeName: labelFont, NSForegroundColorAttributeName: labelTextColor]);
            }
        }
    }
    ...
    public override func renderGridLines(#context: CGContext)
    {
        ...
        var _whilefillRect = CGRect();
        
        var detalY0 = CGFloat();
        var detalY1 = CGFloat();
        var valueToPixelMatrix = transformer.valueToPixelMatrix;
        
        var position = CGPoint(x: 0.0, y: 50.0);
        
        // draw the horizontal grid 画水平线
        for (var i = 0, count = _yAxis.entryCount; i < count; i++)
        {
            ...
            if i == 0{
                position.x = 0.0;
                position.y = CGFloat(_yAxis.entries[i]) + 6;
                position = CGPointApplyAffineTransform(position, valueToPixelMatrix)
                ...
                _gridLineBuffer[1].y = position.y ;
                CGContextStrokeLineSegments(context, _gridLineBuffer, 2);
                detalY0 = _gridLineBuffer[1].y
//                print("Y0:\(_gridLineBuffer[1].y)")
            }
            
            if i == 1{
                detalY1 = _gridLineBuffer[1].y
//                print("Y1:\(_gridLineBuffer[1].y)")
                //画白色填充
                _whilefillRect.origin.x = _gridLineBuffer[0].x
                _whilefillRect.origin.y = _gridLineBuffer[0].y
                _whilefillRect.size.width = 100000
                _whilefillRect.size.height = detalY0 - detalY1
                //保存Y坐标给显示时间时使用
                let defaults = NSUserDefaults.standardUserDefaults()
                defaults.setObject(_gridLineBuffer[0].y, forKey:"gridLineBuffer")
            
                //                trans.rectValueToPixel(&_whilefillRect);
                CGContextSetFillColorWithColor(context, UIColor.whiteColor().CGColor);
                CGContextFillRect(context, _whilefillRect)
            }
